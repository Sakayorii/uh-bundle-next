...
export default async function build(dev = Dev, log = true) {
    ...
    const REPO = 'sakayorii/tacet-bundle-next'
...
async function generateAssets() {
    ...
    for (const file of await readdir(AssetsDir)) {
        ...
        promises.push(
            writeFile(
                path2,
                `import{registerAsset}from'@tacet-mod/assets';import uri from${uriPath};const ${name}=registerAsset({name:'${name}',type:${type},uri});export { ${name} as default }`,
            ),
        )
    }
